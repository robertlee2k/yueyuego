...
//		"ishgtb",
	...
	public static String findNominalAttribs(BaseInstances data){
		return InstanceUtility.returnAttribsPosition(data,NOMINAL_ATTRIBS);
	}
	
	// 从All Transaction Data中删除无关字段 (tradeDate到均线策略之前）
	protected static BaseInstances prepareTransData(BaseInstances allData)
			throws Exception {
		BaseInstances result = InstanceUtility.removeAttribs(allData,TRANS_DATA_NOT_SAVED_IN_ARFF);// "3-9");
		return result;
	}
	...
	protected static BaseInstances getTransLeftPartFromAllTransaction(BaseInstances allData)
			throws Exception {
		return InstanceUtility.keepAttributes(allData,TRANS_DATA_LEFT_PART);
	}
	
	// 为原始的Arff文件加上计算属性
	public static BaseInstances addCalculateAttribute(BaseInstances data) throws Exception {
		BaseInstances result = new DataInstances(data, 0);
...
//				{ "指数bias5", "指数bias10", "指数bias20", "指数bias30", "指数bias60" } };
		...
		// 为每一行数据处理
		for (int i = 0; i < row; i++) {
			BaseInstance oneRow = data.instance(i);
			for (int x = 0; x < biasAttName.length; x++) {
				for (int j = 0; j < biasAttName[x].length; j++) {
					BaseAttribute attribute = data.attribute(biasAttName[x][j]);
			...
			for (int n = 0; n < data.numAttributes() - 1; n++) {
				BaseAttribute att = data.attribute(n);
				BaseAttribute newRowAtt=result.attribute(n);
...
//				}
	...
	public static BaseInstances validateAttributeNames(BaseInstances data,String[] standardFormat) throws Exception {
...
//						+ standardFormat[i] + "]");
	...
	protected static boolean belongToIndex(BaseInstance curr, String indexName) {
		BaseAttribute hsAtt = curr.dataset().attribute(indexName);
	...
	// 判断是否为沪深300
	public static boolean isHS300(BaseInstance curr) {
		return belongToIndex(curr, IS_HS300);
	}

	// 判断是否为中证500
	public static boolean isZZ500(BaseInstance curr) {
		return belongToIndex(curr, IS_ZZ500);
	}

	// 判断是否为上证50
	public static boolean isSZ50(BaseInstance curr) {
	 ...
	 * @return
	 */
	public static boolean checkSumBeforeMerge(BaseInstance leftCurr,
			BaseInstance rightCurr, BaseAttribute leftMA, BaseAttribute rightMA,
			BaseAttribute leftBias5, BaseAttribute rightBias5) {
